...
TEST(DFA, ParseDFA)
{
  ...
  EXPECT_EQ(start_state, dfa::State{"q1"});
  ...
  for (const auto& transition : expected_transitions)
  {
    ...
    if (iter_1 != transitions.end())
    {
      ...
      {
        EXPECT_EQ(iter_2->second, dfa::State{transition.s2});
      }
    }
  }
}
...
TEST(NFA, ConvertToDFA)
{
  ...
  const auto& start_state = dfa.GetStartState();
  const auto& final_states = dfa.GetFinalStates();
  EXPECT_EQ(start_state, dfa::State({"q0", "q1"}));
  EXPECT_NE(final_states.find(dfa::State{"q0", "q1"}), final_states.end());
  EXPECT_NE(final_states.find(dfa::State{"q0", "q1", "q2"}), final_states.end());
...
{
  dfa::State s1{"q0", "q1", "q2"};
  dfa::State s2{"q0", "q2", "q1"};
  dfa::State s3{"q1", "q0", "q2"};
  dfa::State s4{"q1", "q2", "q0"};
  dfa::State s5{"q2", "q0", "q1"};
  dfa::State s6{"q2", "q1", "q0"};

  dfa::StateSet set{s1, s2, s3, s4, s5, s6};
